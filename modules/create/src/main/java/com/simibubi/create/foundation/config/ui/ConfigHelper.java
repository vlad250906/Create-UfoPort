...
public class ConfigHelper {
	...
	private static EnumMap<ModConfig.Type, ModConfig> findModConfigsUncached(String modID) {
		...
		EnumMap<ModConfig.Type, Set<ModConfig>> configSets = new EnumMap<>(ModConfig.Type.class);
		for (var type : ModConfig.Type.values()) {
            configSets.put(type, Collections.synchronizedSet(new LinkedHashSet<>()));
        }
		
		configSets.forEach((type, modConfigs) -> {
	...
	@Nullable
	public static ModConfigSpec findForgeConfigSpecFor(ModConfig.Type type, String modID) {
