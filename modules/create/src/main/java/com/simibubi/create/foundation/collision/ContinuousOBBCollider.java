...
public class ContinuousOBBCollider extends OBBCollider {
	...
	public static ContinuousSeparationManifold separateBBs(Vec3 cA, Vec3 cB, Vec3 eA, Vec3 eB, Matrix3d m,
			Vec3 motion) {
	...
	static boolean separate(ContinuousSeparationManifold mf, Vec3 axis, double TL, double rA, double rB,
		...
		// boolean isBestSeperation = discreteCollision && checkCount == 5; // Debug
		...
		if (dot != 0 && discreteCollision) {
			...
			if (!cross.equals(Vec3.ZERO)) {
				...
				stepSeparationVec = sepVec.subtract(axisPlane.scale(sepVec.dot(stepPlane) / axisPlane.dot(stepPlane)));
		...
		if (isBestSeperation) {
			...
			mf.collisionPosition = normalizedAxis
					.scale(signum(TL) * (axisOfObjA ? -rB : -rB) - signum(seperation) * .125f);
		}
