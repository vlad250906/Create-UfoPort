...
public enum TrackShape implements StringRepresentable {
	NONE("", Vec3.ZERO), ZO("z_ortho", new Vec3(0, 0, 1)), XO("x_ortho", new Vec3(1, 0, 0)),
	PD("diag", new Vec3(1, 0, 1)), ND("diag_2", new Vec3(-1, 0, 1)),
	...
	TN("teleport", 180, new Vec3(0, 0, -1), new Vec3(0, 1, 0)), TS("teleport", 0, new Vec3(0, 0, 1), new Vec3(0, 1, 0)),
	...
	static {
		...
		clockwise.putAll(ImmutableMap.<TrackShape, TrackShape>builder().put(PD, ND).put(ND, PD).put(XO, ZO).put(ZO, XO)
				.put(AE, AS).put(AS, AW).put(AW, AN).put(AN, AE).put(CR_PDX, CR_NDZ).put(CR_NDX, CR_PDZ)
				.put(CR_PDZ, CR_NDX).put(CR_NDZ, CR_PDX).build());
	}
