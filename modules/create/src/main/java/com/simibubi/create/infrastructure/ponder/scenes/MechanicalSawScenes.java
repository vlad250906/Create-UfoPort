...
public class MechanicalSawScenes {
	...
	public static void processing(SceneBuilder scene, SceneBuildingUtil util) {
		...
		logItem = scene.world.createItemEntity(util.vector.topOf(sawPos).add(0.5, -.1, 0),
				util.vector.of(0.05, 0.18, 0), strippedLog);
		scene.idle(12);
		...
		logItem = scene.world.createItemEntity(util.vector.topOf(sawPos).add(-0.5, -.1, 0),
				util.vector.of(-0.05, 0.18, 0), planks);
		scene.idle(22);
		...
		ElementLink<WorldSectionElement> otherBeltSection = scene.world.showIndependentSection(otherBelt,
				Direction.WEST);
	...
	public static void treeCutting(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.hideSection(util.select.fromTo(2, 1, 2, 1, 1, 3).substract(util.select.position(breakingPos)),
				Direction.WEST);
		scene.idle(10);
		scene.overlay.showSelectionWithText(util.select.position(breakingPos), 90).attachKeyFrame()
	...
	public static void contraption(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.showSection(util.select.layer(0).add(util.select.position(3, 1, 1))
				.add(util.select.position(1, 1, 2)).add(util.select.position(2, 1, 4)), Direction.UP);
		...
		scene.world.setKineticSpeed(saws, 0);
		scene.idle(10);
		scene.overlay.showControls(new InputWindowElement(util.vector.topOf(3, 2, 2), Pointing.DOWN)
				.withItem(new ItemStack(Blocks.OAK_LOG)), 60);
		scene.idle(20);
	}
