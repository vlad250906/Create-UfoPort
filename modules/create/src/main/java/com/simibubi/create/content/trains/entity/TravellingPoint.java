...
public class TravellingPoint {
	...
	public static interface ITrackSelector extends
			BiFunction<TrackGraph, Pair<Boolean, List<Entry<TrackNode, TrackEdge>>>, Entry<TrackNode, TrackEdge>> {
	};
	...
	public ITrackSelector random() {
		return (graph, pair) -> pair.getSecond().get(Create.RANDOM.nextInt(pair.getSecond().size()));
	}
	...
	public double travel(TrackGraph graph, double distance, ITrackSelector trackSelector,
		...
		Double blockedLocation = edgeTraversedFrom(graph, forward, signalListener, turnListener, prevPos,
				collectedDistance);
		...
		if (forward) {
			...
			while (position > edgeLength) {
				...
				for (Entry<TrackNode, TrackEdge> entry : graph.getConnectionsFrom(node2).entrySet()) {
				...
				if (entry.getValue().getLength() == 0
						&& portalListener.test(Couple.create(node2.getLocation(), entry.getKey().getLocation()))) {
					traveled -= position - edgeLength;
					position = edgeLength;
					blocked = true;
					break;
			...
			while (position < 0) {
				...
				for (Entry<TrackNode, TrackEdge> entry : graph.getConnectionsFrom(node1).entrySet()) {
					TrackNode newNode = entry.getKey();
					if (newNode == node2)
						continue;
				...
				if (entry.getValue().getLength() == 0
						&& portalListener.test(Couple.create(entry.getKey().getLocation(), node1.getLocation()))) {
					traveled -= position;
					position = 0;
					blocked = true;
					break;
				...
				blockedLocation = edgeTraversedFrom(graph, forward, signalListener, turnListener, edgeLength,
						collectedDistance);
	...
	public static TravellingPoint read(CompoundTag tag, TrackGraph graph, DimensionPalette dimensions) {
		...
		Couple<TrackNode> locs = tag.contains("Nodes") ? Couple
				.deserializeEach(tag.getList("Nodes", Tag.TAG_COMPOUND), c -> TrackNodeLocation.read(c, dimensions))
				.map(graph::locateNode) : Couple.create(null, null);

		if (locs.either(Objects::isNull))
			return new TravellingPoint(null, null, null, 0, false);

		double position = tag.getDouble("Position");
		return new TravellingPoint(locs.getFirst(), locs.getSecond(),
				graph.getConnectionsFrom(locs.getFirst()).get(locs.getSecond()), position,
				tag.getBoolean("UpsideDown"));
	}
