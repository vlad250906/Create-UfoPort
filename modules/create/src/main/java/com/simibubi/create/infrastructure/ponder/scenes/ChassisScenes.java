...
public class ChassisScenes {
	...
	public static void linearGroup(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showSelectionWithText(wrong1, 80).colored(PonderPalette.RED).placeNearTarget()
	...
	public static void linearAttachement(SceneBuilder scene, SceneBuildingUtil util) {
		...
		InputWindowElement input = new InputWindowElement(util.vector.blockSurface(chassisPos, Direction.WEST),
				Pointing.LEFT).rightClick().withItem(new ItemStack(Items.SLIME_BALL));
		...
		scene.overlay.showSelectionWithText(column3, 80).colored(PonderPalette.GREEN)
		...
		scene.overlay.showCenteredScrollInput(chassisPos, Direction.NORTH, 50);
		scene.overlay.showControls(
				new InputWindowElement(blockSurface, Pointing.UP).whileCTRL().rightClick().withWrench(), 50);
		...
		scene.idle(90);
		scene.overlay.showControls(
				new InputWindowElement(glueSurface, Pointing.DOWN).rightClick().withItem(AllItems.SUPER_GLUE.asStack()),
				30);
		scene.idle(7);
		scene.overlay.chaseBoundingBoxOutline(PonderPalette.GREEN, glueSurface,
				new AABB(util.grid.at(1, 2, 2)).expandTowards(0, 0, -1), 40);
		scene.idle(20);
		ElementLink<WorldSectionElement> gluedPlank = scene.world.showIndependentSection(util.select.position(3, 3, 1),
				Direction.SOUTH);
		...
		scene.addKeyframe();
		ElementLink<WorldSectionElement> chain = scene.world.showIndependentSection(util.select.position(2, 7, 2),
				Direction.DOWN);
	...
	public static void radial(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.modifyBlock(chassisPos, s -> s.setValue(RadialChassisBlock.STICKY_EAST, true)
				.setValue(RadialChassisBlock.STICKY_NORTH, true).setValue(RadialChassisBlock.STICKY_SOUTH, true),
				false);
		scene.effects.superGlue(chassisPos, Direction.EAST, false);
		...
		scene.overlay.showSelectionWithText(obj.getValue(), 60).colored(PonderPalette.GREEN)
		...
		scene.overlay.showOutline(PonderPalette.GREEN, r2, r2.copy().substract(util.select.fromTo(0, 2, 0, 1, 2, 4)),
				80);
		scene.markAsFinished();
		scene.overlay.showSelectionWithText(ignored, 80).colored(PonderPalette.RED)
	...
	public static void superGlue(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showText(80).text("Super Glue groups blocks together into moving contraptions").placeNearTarget()
				.attachKeyFrame().pointAt(util.vector.topOf(util.grid.at(2, 1, 2)));
		scene.idle(70);

		scene.overlay.showControls(new InputWindowElement(util.vector.topOf(2, 2, 3), Pointing.DOWN)
				.withItem(AllItems.SUPER_GLUE.asStack()).rightClick(), 40);
		scene.idle(6);
		...
		scene.overlay.showControls(
				new InputWindowElement(util.vector.blockSurface(util.grid.at(1, 1, 2), Direction.DOWN), Pointing.UP)
						.withItem(AllItems.SUPER_GLUE.asStack()).rightClick(),
				40);
		scene.idle(6);
		...
		scene.overlay.showControls(
				new InputWindowElement(util.vector.blockSurface(util.grid.at(3, 1, 1), Direction.UP), Pointing.DOWN)
						.withItem(AllItems.SUPER_GLUE.asStack()).leftClick(),
				40);
		scene.idle(50);
		...
		scene.overlay.showOutline(PonderPalette.GREEN, cogs, util.select.fromTo(4, 2, 2, 1, 2, 2)
				.add(util.select.fromTo(2, 3, 3, 2, 2, 3)).add(util.select.fromTo(1, 3, 2, 0, 3, 2)), 70);
