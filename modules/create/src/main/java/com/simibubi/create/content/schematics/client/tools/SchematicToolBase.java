...
public abstract class SchematicToolBase implements ISchematicTool {
	...
	public void updateTargetPos() {
		...
		if (schematicHandler.isDeployed()) {
			...
			PredicateTraceResult result = RaycastHelper.rayTraceUntil(start, end,
		...
		if (selectIgnoreBlocks) {
			...
			selectedPos = BlockPos
					.containing(player.getEyePosition(pt).add(player.getLookAngle().scale(selectionRange)));
			if (snap)
	...
	public void renderOnSchematic(PoseStack ms, SuperRenderTypeBuffer buffer) {
		...
		if (renderSelectedFace) {
			outline.getParams().highlightFace(selectedFace).withFaceTextures(AllSpecialTextures.CHECKERED,
