...
public class ChapterLabel extends AbstractSimiWidget {
	...
	public ChapterLabel(PonderChapter chapter, int x, int y, BiConsumer<Integer, Integer> onClick) {
		...
		this.button = new PonderButton(x + 4, y + 4, 30, 30).showing(chapter).withCallback(onClick);
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		graphics.drawString(Minecraft.getInstance().font, chapter.getTitle(), getX() + 50, getY() + 20,
				Theme.i(Theme.Key.TEXT_ACCENT_SLIGHT), false);
