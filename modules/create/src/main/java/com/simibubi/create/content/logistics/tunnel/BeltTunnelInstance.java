...
public class BeltTunnelInstance extends BlockEntityInstance<BeltTunnelBlockEntity> implements DynamicInstance {
	...
	public BeltTunnelInstance(MaterialManager materialManager, BeltTunnelBlockEntity blockEntity) {
		...
		Instancer<FlapData> model = materialManager.defaultSolid().material(AllMaterialSpecs.FLAPS)
		...
		blockEntity.flaps.forEach((direction, flapValue) -> {
			...
			for (int segment = 0; segment <= 3; segment++) {
				...
				key.setPosition(getInstancePosition()).setSegmentOffset(segmentOffset, 0, 0).setBlockLight(blockLight)
						.setSkyLight(skyLight).setHorizontalAngle(horizontalAngle).setFlapness(flapness)
						.setFlapScale(flapScale).setPivotVoxelSpace(0, 10, 1).setIntensity(intensity);
	...
	@Override
	public void remove() {
		tunnelFlaps.values().stream().flatMap(Collection::stream).forEach(InstanceData::delete);
	}
}
