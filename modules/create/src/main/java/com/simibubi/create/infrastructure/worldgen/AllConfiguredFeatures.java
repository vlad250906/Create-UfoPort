...
public class AllConfiguredFeatures {
	...
	public static void bootstrap(BootstrapContext<ConfiguredFeature<?, ?>> ctx) {
		...
		List<TargetBlockState> zincTargetStates = List.of(
				OreConfiguration.target(stoneOreReplaceables, AllBlocks.ZINC_ORE.get().defaultBlockState()),
				OreConfiguration.target(deepslateOreReplaceables,
						AllBlocks.DEEPSLATE_ZINC_ORE.get().defaultBlockState()));

		register(ctx, ZINC_ORE, Feature.ORE, new OreConfiguration(zincTargetStates, 12));

		List<LayerPattern> overworldLayerPatterns = List.of(AllLayerPatterns.SCORIA.get(),
				AllLayerPatterns.CINNABAR.get(), AllLayerPatterns.MAGNETITE.get(), AllLayerPatterns.MALACHITE.get(),
				AllLayerPatterns.LIMESTONE.get(), AllLayerPatterns.OCHRESTONE.get());

		register(ctx, STRIATED_ORES_OVERWORLD, AllFeatures.LAYERED_ORE.get(),
				new LayeredOreConfiguration(overworldLayerPatterns, 32, 0));

		List<LayerPattern> netherLayerPatterns = List.of(AllLayerPatterns.SCORIA_NETHER.get(),
				AllLayerPatterns.SCORCHIA_NETHER.get());

		register(ctx, STRIATED_ORES_NETHER, AllFeatures.LAYERED_ORE.get(),
