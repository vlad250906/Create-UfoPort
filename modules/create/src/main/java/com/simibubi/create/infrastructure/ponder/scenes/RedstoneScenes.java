...
public class RedstoneScenes {
	...
	public static void sticker(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showText(60).text("Stickers are ideal for Redstone-controlled block attachment").attachKeyFrame()
	...
	public static void pulseExtender(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(10);
		scene.overlay.showText(60).text("Using the value panel, the discharge time can be configured").attachKeyFrame()
				.placeNearTarget().pointAt(circuitTop);
		scene.world.modifyBlockEntityNBT(util.select.position(circuitPos), PulseExtenderBlockEntity.class,
	...
	public static void analogLever(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showText(60).attachKeyFrame()
				.text("Right-click while Sneaking to decrease the power output again").placeNearTarget()
				.pointAt(leverVec);
		scene.idle(70);
	...
	public static void nixieTube(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(centerTube.add(1, .35, 0), Pointing.DOWN).rightClick().withItem(clipboard), 40);
		scene.idle(7);
		...
		for (int i = 0; i < 3; i++) {
			...
			scene.world.modifyBlockEntityNBT(util.select.position(3 - i, 1, 3), NixieTubeBlockEntity.class, nbt -> {
				...
				nbt.putString("CustomText", Component.Serializer.toJson(component, Create.getRegistryAccess()));
		...
		InputWindowElement input = new InputWindowElement(util.vector.blockSurface(util.grid.at(3, 1, 3), Direction.UP),
				Pointing.DOWN).withItem(new ItemStack(Items.BLUE_DYE));
		...
		scene.world.setBlocks(util.select.fromTo(1, 1, 3, 3, 1, 3), AllBlocks.NIXIE_TUBES.get(DyeColor.BLUE)
				.getDefaultState().setValue(NixieTubeBlock.FACING, Direction.WEST), false);
		scene.idle(10);
	...
	public static void redstoneLink(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showText(70).colored(PonderPalette.GREEN)
				.text("Receivers emit the redstone power of transmitters within 128 blocks").placeNearTarget()
				.pointAt(link2Vec);
		scene.idle(80);
		scene.world.toggleRedstonePower(redstone);
		scene.idle(5);
