...
public class DynamicComponent {
	...
	public void read(Level level, BlockPos pos, CompoundTag nbt) {
		...
		try {
			parsedCustomText = Component.Serializer.fromJson(nbt.getString("CustomText"), Create.getRegistryAccess());
	...
	public void write(CompoundTag nbt) {
		...
		nbt.putString("CustomText", Component.Serializer.toJson(parsedCustomText, Create.getRegistryAccess()));
	}
	...
	public static Component parseCustomText(Level level, BlockPos pos, JsonElement customText) {
		...
		try {
			return ComponentUtils.updateForEntity(getCommandSource(serverLevel, pos),
				Component.Serializer.fromJson(customText, Create.getRegistryAccess()), null, 0);
		} catch (JsonParseException e) {
			return null;
