...
public interface ValueSettingsBehaviour extends ClipboardCloneable {
	...
	public static record ValueSettings(int row, int value) {
		...
		public MutableComponent format() {
			return Lang.number(value).component();
		}
	...
	default void playFeedbackSound(BlockEntityBehaviour origin) {
		origin.getWorld().playSound(null, origin.getPos(), SoundEvents.ITEM_FRAME_ADD_ITEM, SoundSource.BLOCKS, 0.25f,
				2f);
		origin.getWorld().playSound(null, origin.getPos(), SoundEvents.NOTE_BLOCK_IRON_XYLOPHONE.value(),
				SoundSource.BLOCKS, 0.03f, 1.125f);
	}
