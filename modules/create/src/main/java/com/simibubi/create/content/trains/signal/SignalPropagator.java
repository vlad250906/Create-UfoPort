...
public class SignalPropagator {
	...
	public static void walkSignals(TrackGraph graph, SignalBoundary signal, boolean front,
		...
		Couple<TrackEdge> startEdges = startNodes.mapWithParams((l1, l2) -> graph.getConnectionsFrom(l1).get(l2),
				startNodes.swap());
		...
		SignalBoundary immediateBoundary = startEdge.getEdgeData().next(EdgePointType.SIGNAL,
				signal.getLocationOn(startEdge));
	...
	private static void walkSignals(TrackGraph graph, List<Couple<TrackNode>> frontier,
		...
		while (!frontier.isEmpty()) {
			...
			EdgeWalk: for (Entry<TrackNode, TrackEdge> entry : graph.getConnectionsFrom(currentNode).entrySet()) {
