...
public class Navigation {
	...
	public void tick(Level level) {
		...
		if (train.graph != null) {
			...
			TravellingPoint leadingPoint = !destinationBehindTrain ? train.carriages.get(0).getLeadingPoint()
				...
				signalScout.travel(train.graph, (distanceToDestination + 50) * speedMod, controlSignalScout(),
						(distance, couple) -> {
							...
							if (boundary == destination
									&& ((GlobalStation) boundary).canApproachFrom(nodes.getSecond()))
								return true;
							...
							boolean occupied = !train.manualTick && (signal.isForcedRed(nodes.getSecond())
							...
							if (crossSignalTracked) {
								...
								if (occupied) { // Section is occupied, but wait at the cross signal that started the
												// chain
		...
		if (distanceToNextCurve != -1) {
			...
			double targetTurnSpeed = distanceToNextCurve > slowingDistance ? topSpeed * speedMod
					: turnTopSpeed * speedMod;
	...
	public DiscoveredPath findPathTo(ArrayList<GlobalStation> destinations, double maxCost) {
		...
		for (boolean forward : Iterate.trueAndFalse) {
			...
			TravellingPoint initialPoint = forward ? train.carriages.get(0).getLeadingPoint()
					: train.carriages.get(train.carriages.size() - 1).getTrailingPoint();
			TrackEdge initialEdge = forward ? initialPoint.edge
					: graph.getConnectionsFrom(initialPoint.node2).get(initialPoint.node1);

			search(Double.MAX_VALUE, maxCost, forward, destinations,
					(distance, cost, reachedVia, currentEntry, globalStation) -> {
						for (GlobalStation destination : destinations) {
							if (globalStation == destination) {
								...
								results.set(forward, new DiscoveredPath((forward ? 1 : -1) * distanceToDestination,
										cost, currentPath, destination));
								return true;
							}
						}
						return false;
	...
	public void search(double maxDistance, boolean forward, ArrayList<GlobalStation> destinations,
			StationTest stationTest) {
		search(maxDistance, -1, forward, destinations, stationTest);
	}

	public void search(double maxDistance, double maxCost, boolean forward, ArrayList<GlobalStation> destinations,
			StationTest stationTest) {
		TrackGraph graph = train.graph;
		if (graph == null)
			return;
		...
		for (int i = 0; i < train.carriages.size(); i++) {
			...
			if (i == 0) {
				validTypes.addAll(
						carriage.leadingBogey().type.getValidPathfindingTypes(carriage.leadingBogey().getStyle()));
			} else {
				validTypes.retainAll(
						carriage.leadingBogey().type.getValidPathfindingTypes(carriage.leadingBogey().getStyle()));
			}
			if (carriage.isOnTwoBogeys())
				validTypes.retainAll(
						carriage.trailingBogey().type.getValidPathfindingTypes(carriage.trailingBogey().getStyle()));
		}
			...
			for (TrackEdgePoint point : initialSignalData.getPoints()) {
				...
				if (point instanceof GlobalStation station) {
					...
					if (station.canApproachFrom(initialNode2)
							&& stationTest.test(distanceToNode2, distanceToNode2 + initialPenalty, reachedVia,
			...
			Search: for (Entry<TrackNode, TrackEdge> target : validTargets) {
				...
				if (signalData.hasPoints()) {
					for (TrackEdgePoint point : signalData.getPoints()) {
						...
						if (point instanceof GlobalStation station) {
							...
							if (station.canApproachFrom(newNode)
									&& stationTest.test(newDistance, newDistance + newPenalty, reachedVia,
				...
				if (destinations != null && !destinations.isEmpty()) {
					...
					for (GlobalStation destination : destinations) {
						...
						double currentRemaining = 0.317837245195782 * dMin + 0.414213562373095 * dMid + dMax
								+ destination.position;
						if (node2.getLocation().equals(destinationNode))
							currentRemaining -= newEdge.getLength() * 2; // Correct the distance estimator for station
																			// edge
				...
				frontier.add(new FrontierEntry(newDistance, newPenalty, remainingDist, hasDestination, node2, newNode,
						newEdge));
			}
		}
	}
	...
	private class FrontierEntry implements Comparable<FrontierEntry> {
		...
		public FrontierEntry(double distance, int penalty, double remaining, boolean hasDestination, TrackNode node1,
				TrackNode node2, TrackEdge edge) {
			this.distance = distance;
			this.penalty = penalty;
	...
	public void read(CompoundTag tag, TrackGraph graph, DimensionPalette dimensions) {
		...
		currentPath.clear();
		NBTHelper.iterateCompoundList(tag.getList("Path", Tag.TAG_COMPOUND), c -> currentPath.add(Couple
