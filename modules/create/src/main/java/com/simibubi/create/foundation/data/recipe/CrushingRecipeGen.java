...
public class CrushingRecipeGen extends ProcessingRecipeGen {
	...
	BLAZE_ROD = create(() -> Items.BLAZE_ROD,
			b -> b.duration(100).output(Items.BLAZE_POWDER, 3).output(.25f, Items.BLAZE_POWDER, 3)),

			PRISMARINE_CRYSTALS = create(() -> Items.PRISMARINE_CRYSTALS,
					b -> b.duration(150).output(1f, Items.QUARTZ, 1).output(.5f, Items.QUARTZ, 2).output(.1f,
							Items.GLOWSTONE_DUST, 2)),

			OBSIDIAN = create(() -> Blocks.OBSIDIAN,
					b -> b.duration(500).output(AllItems.POWDERED_OBSIDIAN.get()).output(.75f, Blocks.OBSIDIAN)),

			WOOL = create("wool",
					b -> b.duration(100).require(ItemTags.WOOL).output(Items.STRING, 2).output(.5f, Items.STRING)),

			DIORITE = mineralRecycling(AllPaletteStoneTypes.DIORITE,
					b -> b.duration(350).output(.25f, Items.QUARTZ, 1)),

			CRIMSITE = mineralRecycling(AllPaletteStoneTypes.CRIMSITE, AllItems.CRUSHED_IRON::get,
					() -> Items.IRON_NUGGET, .4f),
			...
			OCHRUM = mineralRecycling(AllPaletteStoneTypes.OCHRUM, AllItems.CRUSHED_GOLD::get, () -> Items.GOLD_NUGGET,
					.2f),

			TUFF = mineralRecycling(AllPaletteStoneTypes.TUFF,
					b -> b.duration(350).output(.25f, Items.FLINT, 1).output(.1f, Items.GOLD_NUGGET, 1)
							.output(.1f, AllItems.COPPER_NUGGET.get(), 1).output(.1f, AllItems.ZINC_NUGGET.get(), 1)
			...
			NETHER_WART = create("nether_wart_block",
					b -> b.duration(150).require(Blocks.NETHER_WART_BLOCK).output(.25f, Items.NETHER_WART, 1)),

			AMETHYST_CLUSTER = create(() -> Blocks.AMETHYST_CLUSTER,
					b -> b.duration(150).output(Items.AMETHYST_SHARD, 7).output(.5f, Items.AMETHYST_SHARD)),

			GLOWSTONE = create(() -> Blocks.GLOWSTONE,
					b -> b.duration(150).output(Items.GLOWSTONE_DUST, 3).output(.5f, Items.GLOWSTONE_DUST)),

			AMETHYST_BLOCK = create(() -> Blocks.AMETHYST_BLOCK,
					b -> b.duration(150).output(Items.AMETHYST_SHARD, 3).output(.5f, Items.AMETHYST_SHARD)),

			LEATHER_HORSE_ARMOR = create(() -> Items.LEATHER_HORSE_ARMOR,
					b -> b.duration(200).output(Items.LEATHER, 2).output(.5f, Items.LEATHER, 2)),

			IRON_HORSE_ARMOR = create(() -> Items.IRON_HORSE_ARMOR,
					b -> b.duration(200).output(Items.IRON_INGOT, 2).output(.5f, Items.LEATHER, 1)
							.output(.5f, Items.IRON_INGOT, 1).output(.25f, Items.STRING, 2)
							.output(.25f, Items.IRON_NUGGET, 4)),

			GOLDEN_HORSE_ARMOR = create(() -> Items.GOLDEN_HORSE_ARMOR,
					b -> b.duration(200).output(Items.GOLD_INGOT, 2).output(.5f, Items.LEATHER, 2)
							.output(.5f, Items.GOLD_INGOT, 2).output(.25f, Items.STRING, 2)
							.output(.25f, Items.GOLD_NUGGET, 8)),

			DIAMOND_HORSE_ARMOR = create(() -> Items.DIAMOND_HORSE_ARMOR,
					b -> b.duration(200).output(Items.DIAMOND, 1).output(.5f, Items.LEATHER, 2)
							.output(.1f, Items.DIAMOND, 3).output(.25f, Items.STRING, 2)),

			GRAVEL = create(() -> Blocks.GRAVEL,
					b -> b.duration(250).output(Blocks.SAND).output(.1f, Items.FLINT).output(.05f, Items.CLAY_BALL)),

			NETHERRACK = create(() -> Blocks.NETHERRACK,
					b -> b.duration(250).output(AllItems.CINDER_FLOUR.get()).output(.5f, AllItems.CINDER_FLOUR.get()))
	...
	protected GeneratedRecipe mineralRecycling(AllPaletteStoneTypes type, Supplier<ItemLike> crushed,
		...
		return mineralRecycling(type,
				b -> b.duration(250).output(chance, crushed.get(), 1).output(chance, nugget.get(), 1));
	}
	...
	protected GeneratedRecipe ore(ItemLike stoneType, Supplier<ItemLike> ore, Supplier<ItemLike> raw,
		...
		return create(ore, b -> {
			ProcessingRecipeBuilder<ProcessingRecipe<?>> builder = b.duration(duration).output(raw.get(),
					Mth.floor(expectedAmount));
	...
	protected GeneratedRecipe rawOre(Supplier<ItemLike> input, Supplier<ItemLike> result, int amount) {
		return create(input, b -> b.duration(400).output(result.get(), amount).output(.75f, AllItems.EXP_NUGGET.get(),
				(result.get() == AllItems.CRUSHED_GOLD.get() ? 2 : 1) * amount));
	}
	...
	protected GeneratedRecipe moddedRawOre(CompatMetals metal, Supplier<ItemLike> result, int amount) {
		...
		return create("raw_" + name + (amount == 1 ? "_ore" : "_block"), b -> {
			...
			return b.duration(400)
					.withCondition(
							ResourceConditions.tagsPopulated(AllTags.forgeItemTag("raw_" + name + suffix)))
					.require(AllTags.forgeItemTag("raw_" + name + suffix)).output(result.get(), amount)
	...
	protected GeneratedRecipe moddedOre(CompatMetals metal, Supplier<ItemLike> result) {
		...
		return create(name + "_ore", b -> {
			String suffix = "_ores";
			return b.duration(400)
					.withCondition(ResourceConditions.tagsPopulated(AllTags.forgeItemTag(name + suffix)))
					.require(AllTags.forgeItemTag(name + suffix)).output(result.get(), 1).output(.75f, result.get(), 1)
					.output(.75f, AllItems.EXP_NUGGET.get(), 1);
		});
	}

	public CrushingRecipeGen(FabricDataOutput output, CompletableFuture<HolderLookup.Provider> registriesFuture) {
