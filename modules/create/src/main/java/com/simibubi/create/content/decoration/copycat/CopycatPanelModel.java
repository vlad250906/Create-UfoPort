...
public class CopycatPanelModel extends CopycatModel {
	...
	protected void emitBlockQuadsInner(BlockAndTintGetter blockView, BlockState state, BlockPos pos,
		...
		Direction facing = state.getOptionalValue(CopycatPanelBlock.FACING).orElse(Direction.UP);
		...
		if (CopycatSpecialCases.isTrapdoorMaterial(material)) {
			((FabricBakedModel) blockRenderer.getBlockModel(material)).emitBlockQuads(blockView, material, pos,
					randomSupplier, context);
			return;
		}
		...
		if (specialCopycatModelState != null) {
			...
			if (blockModel instanceof CopycatModel cm) {
				cm.emitBlockQuadsInner(blockView, state, pos, randomSupplier, context, material, cullFaceRemovalData,
						occlusionData);
				return;
			}
		}
		...
		context.pushTransform(quad -> {
			...
			for (boolean front : Iterate.trueAndFalse) {
				...
				AABB bb = CUBE_AABB.contract(normal.x * contract / 16, normal.y * contract / 16,
						normal.z * contract / 16);
				if (!front)
