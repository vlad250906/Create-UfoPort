...
public class TrackInstance extends BlockEntityInstance<TrackBlockEntity> {
	...
	public void update() {
		...
		remove();
		instances = blockEntity.connections.values().stream().map(this::createInstance).filter(Objects::nonNull)
				.toList();
	...
	private class BezierTrackInstance {
		...
		private BezierTrackInstance(BezierConnection bc) {
			...
			TransformStack.cast(pose).translate(getInstancePosition());

			var mat = materialManager.cutout(RenderType.cutoutMipped()).material(Materials.TRANSFORMED);
			...
			mat.getModel(modelHolder.tie()).createInstances(ties);
			mat.getModel(modelHolder.segment_left()).createInstances(left);
			mat.getModel(modelHolder.segment_right()).createInstances(right);
			...
			for (int i = 1; i < segments.length; i++) {
				...
				ties[modelIndex].setTransform(pose).mulPose(segment.tieTransform.pose())
				...
				for (boolean first : Iterate.trueAndFalse) {
					...
					(first ? this.left : this.right)[modelIndex].setTransform(pose).mulPose(transform.pose())
		...
		private class GirderInstance {
			...
			private GirderInstance(BezierConnection bc) {
				...
				PoseStack pose = new PoseStack();
				TransformStack.cast(pose).translate(getInstancePosition())
						.nudge((int) bc.tePositions.getFirst().asLong());
				...
				for (int i = 1; i < bakedGirders.length; i++) {
					...
					for (boolean first : Iterate.trueAndFalse) {
						...
						beams.get(first)[modelIndex].setTransform(pose).mulPose(beamTransform.pose())
								.mulNormal(beamTransform.normal());
						for (boolean top : Iterate.trueAndFalse) {
							Pose beamCapTransform = segment.beamCaps.get(top).get(first);
							beamCaps.get(top).get(first)[modelIndex].setTransform(pose).mulPose(beamCapTransform.pose())
