...
public class MillingRecipeGen extends ProcessingRecipeGen {
	...
	GRANITE = create(() -> Blocks.GRANITE, b -> b.duration(200)
		...
		GRASS = create(() -> Blocks.GRASS_BLOCK, b -> b.duration(50)
			.output(.25f, Items.WHEAT_SEEDS))
	...
	protected GeneratedRecipe metalOre(String name, ItemEntry<? extends Item> crushed, int duration) {
		return create(name + "_ore", b -> b.duration(duration)
			.withCondition(ResourceConditions.not(ResourceConditions.tagsPopulated(TagKey.create(Registries.ITEM, ResourceLocation.fromNamespaceAndPath("c", name + "_ores")))))
			.require(AllTags.forgeItemTag(name + "_ores"))
			.output(crushed.get()));
	}

	public MillingRecipeGen(FabricDataOutput output, CompletableFuture<HolderLookup.Provider> registriesFuture) {
