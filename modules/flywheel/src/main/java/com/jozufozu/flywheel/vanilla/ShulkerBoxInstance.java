...
public class ShulkerBoxInstance extends BlockEntityInstance<ShulkerBoxBlockEntity> implements DynamicInstance {
	...
	public ShulkerBoxInstance(MaterialManager materialManager, ShulkerBoxBlockEntity blockEntity) {
		...
		tstack.translate(getInstancePosition()).scale(0.9995f).translateAll(0.00025).centre().multiply(rotation)
				.unCentre();
	...
	public void beginFrame() {
		...
		tstack.pushPose().centre().multiply(spin).unCentre().translateY(progress * 0.5f);
	...
	private ModelData makeBaseInstance() {
		return materialManager.cutout(RenderType.entityCutoutNoCull(Sheets.SHULKER_SHEET))
				.material(Materials.TRANSFORMED).model("base_" + texture.contents().name(), this::makeBaseModel)
				.createInstance();
	}

	private ModelData makeLidInstance() {
		return materialManager.cutout(RenderType.entityCutoutNoCull(Sheets.SHULKER_SHEET))
				.material(Materials.TRANSFORMED).model("lid_" + texture.contents().name(), this::makeLidModel)
				.createInstance();
	}

	private ModelPart makeBaseModel() {
		return ModelPart.builder("shulker_base", 64, 64).sprite(texture).cuboid().textureOffset(0, 28).size(16, 8, 16)
				.invertYZ().endCuboid().build();
	}

	private ModelPart makeLidModel() {
		return ModelPart.builder("shulker_lid", 64, 64).sprite(texture).cuboid().size(16, 12, 16).invertYZ().endCuboid()
				.build();
	}
