...
public class InstanceWorld {
	...
	public InstanceWorld(Engine engine, InstanceManager<Entity> entityInstanceManager,
			InstanceManager<BlockEntity> blockEntityInstanceManager) {
		this.engine = engine;
	 ...
	 * <p>
	 * Check and shift the origin coordinate. <br>
	...
	public void tick() {
		...
		blockEntityInstanceManager.tick(taskEngine, renderViewEntity.getX(), renderViewEntity.getY(),
				renderViewEntity.getZ());
		entityInstanceManager.tick(taskEngine, renderViewEntity.getX(), renderViewEntity.getY(),
				renderViewEntity.getZ());
	}
	...
	public void loadEntities(ClientLevel world) {
		...
		// Entities are loaded with the world, so when chunks are reloaded they need to
		// be re-added.
		ClientLevelExtension.cast(world).flywheel$getAllLoadedEntities().forEach(entityInstanceManager::add);
	}
