...
public class InstancedMaterial<D extends InstanceData> implements Material<D> {
	...
	/**
	 * Get an instancer for the given model. Calling this method twice with the same
	 * key will return the same instancer.
	 *
	 * @param key           An object that uniquely identifies the model.
	 * @param modelSupplier A factory that creates the IModel that you want to
	 *                      render.
	 * @return An instancer for the given model, capable of rendering many copies
	 *         for little cost.
	 */
	@Override
	...
	public boolean nothingToRender() {
		return models.size() > 0 && models.values().stream().allMatch(GPUInstancer::isEmpty);
	}
	 ...
	 */
	public void clear() {
		models.values().forEach(GPUInstancer::clear);
	}
