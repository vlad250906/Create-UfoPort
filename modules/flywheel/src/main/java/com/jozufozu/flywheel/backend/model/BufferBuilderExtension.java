...
/**
 * Duck interface used on {@link BufferBuilder} to provide lower level access to
 * the backing memory.
 *
...
public interface BufferBuilderExtension {
	...
	VertexFormat flywheel$getFormat();
	
	VertexFormat.Mode flywheel$getMode();
	 ...
	 */
	void flywheel$injectForRender(ByteBufferBuilder buffer, VertexFormat format, int vertexCount);

	/**
	 * Appends the remaining bytes from the given buffer to this BufferBuilder.
	 * 
	 * @param buffer The buffer from which to copy bytes.
	 * @throws IllegalStateException    If this BufferBuilder is not started or is
	 *                                  the process of writing a vertex
	 * @throws IllegalArgumentException If the given buffer does not contain a whole
	 *                                  number of vertices
	 */
