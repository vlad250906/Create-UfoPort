...
public class GridAlignedBB implements ImmutableBox {
	...
	public static GridAlignedBB from(SectionPos pos) {
		return new GridAlignedBB(pos.minBlockX(), pos.minBlockY(), pos.minBlockZ(), pos.maxBlockX() + 1,
				pos.maxBlockY() + 1, pos.maxBlockZ() + 1);
	}

	public static GridAlignedBB from(BlockPos start, BlockPos end) {
		return new GridAlignedBB(start.getX(), start.getY(), start.getZ(), end.getX() + 1, end.getY() + 1,
				end.getZ() + 1);
	}
	...
	public void mirrorAbout(Direction.Axis axis) {
		Vec3i axisVec = Direction.get(Direction.AxisDirection.POSITIVE, axis).getNormal();
	...
	/**
	 * Grow this bounding box to have power of 2 side length, scaling from the
	 * center.
	 */
	...
	/**
	 * Grow this bounding box to have power of 2 side lengths, scaling from the
	 * minimum coords.
	 */
	...
	public boolean sameAs(ImmutableBox other) {
		return minX == other.getMinX() && minY == other.getMinY() && minZ == other.getMinZ() && maxX == other.getMaxX()
				&& maxY == other.getMaxY() && maxZ == other.getMaxZ();
	}

	@Override
	public boolean sameAs(AABB other) {
		return minX == Math.floor(other.minX) && minY == Math.floor(other.minY) && minZ == Math.floor(other.minZ)
				&& maxX == Math.ceil(other.maxX) && maxY == Math.ceil(other.maxY) && maxZ == Math.ceil(other.maxZ);
	}
	...
	public boolean contains(ImmutableBox other) {
		return other.getMinX() >= this.minX && other.getMaxX() <= this.maxX && other.getMinY() >= this.minY
				&& other.getMaxY() <= this.maxY && other.getMinZ() >= this.minZ && other.getMaxZ() <= this.maxZ;
	}
